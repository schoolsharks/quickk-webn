...
const Recommendation: React.FC = () => {
  ...
  const [selectedUser, setSelectedUser] = useState<RecommendationUser | null>(null);
  ...
  const loadRecommendations = useCallback(async (page: number, reset: boolean = false) => {
    try {
      const params: any = {
        page,
        limit: 10,
      ...
      const result = await getRecommendations(params).unwrap() as RecommendationResponse;
      ...
      if (result.success) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(`Filtered ${result.data.recommendations.length - newRecommendations.length} current user entries from recommendations`);
  ...
  }, [loadRecommendations, pagination.hasMore, pagination.currentPage, isLoading]);

  const handleActionClick = useCallback((user: RecommendationUser, event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    <Box>
      ...
      <Box p={3} pb={1}>
        <Box display="flex" alignItems="center" justifyContent="space-between" mb={1}>
          <Typography variant="h2" color={theme.palette.text.primary} fontSize="24px" fontWeight={600}>
            Suggestions for you
          </Typography>
          <Typography 
            variant="body2" 
