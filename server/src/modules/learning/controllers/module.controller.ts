...
export const getModuleById = async (
  ...
  try {
    ...
    next(
      new AppError(
        "An unexpected error occurred while fetching the module",
...
export const checkQuestionResponse = async (
  ...
  try {
    ...
    if (!questionId) {
      return next(
        new AppError("questionId is required", StatusCodes.BAD_REQUEST)
      );
    }
    if (typeof userResponse === "undefined") {
      return next(
        new AppError("response is required", StatusCodes.BAD_REQUEST)
      );
    }
    ...
    next(
      new AppError(
        "Error checking question response",
        StatusCodes.INTERNAL_SERVER_ERROR
      )
    );
  }
...
export const submitLearningResponse = async (
  ...
  try {
    ...
    if (!questionResponse) {
      return next(
        new AppError("questionResponse is required", StatusCodes.BAD_REQUEST)
      );
    }
    if (!refId) {
    ...
    res.status(StatusCodes.CREATED).json({
      ...
      data: result,
      message:
        "Response submitted successfully. awarded stars: " + question.score,
    });
  } catch (error) {
    console.error("Error in submitLearningResponse:", error);
    next(
      new AppError(
        "An unexpected error occurred while submitting the learning response",
        StatusCodes.INTERNAL_SERVER_ERROR
      )
    );
  }
...
export const getModuleComplete = async (
  ...
  try {
    ...
    if (!currentModule) {
      return next(
        new AppError("Current module not found", StatusCodes.NOT_FOUND)
      );
    }
    ...
    const learning = await learningService.getLearningByModuleId(
      currentModuleId
    );
    ...
    if (!learning) {
      return next(
        new AppError(
          "Learning not found for this module",
    ...
    if (moduleIndex === -1) {
      return next(
        new AppError("Module not found in learning", StatusCodes.NOT_FOUND)
      );
    }

    // Determine next module and response data
    let nextModuleId = null;
    let message: string | undefined = "All modules completed";

    // Check if there is a next module in the current learning
    if (moduleIndex < learning.modules.length - 1) {
      nextModuleId = learning.modules[moduleIndex + 1];
      message = undefined;
    } else if (typeof learning.week === "number") {
      // No next module in current learning, check for next learning
    ...
    const responseData = {
      ...
      totalStarsAwarded,
      ...(nextModuleId &&
        moduleIndex < learning.modules.length - 1 && {
    ...
    console.error("Error in getModuleComplete:", error);
    next(
      new AppError(
        "An unexpected error occurred while fetching the next module",
        StatusCodes.INTERNAL_SERVER_ERROR
      )
    );
  }
...
export const markVideoCompleted = async (
  ...
  try {
    ...
    const result = await moduleService.markVideoCompleted(userId, videoId);
    ...
    next(
      new AppError(
        "Error marking video as completed",
        StatusCodes.INTERNAL_SERVER_ERROR
      )
    );
  }
...
export const updateModule = async (
  ...
  try {
    const { moduleId, content, title, assessment, flashcards, type, duration } =
      req.body;
    ...
    if (!moduleId) {
      return next(
        new AppError("moduleId is required", StatusCodes.BAD_REQUEST)
      );
    }
    if (!type) {
      return next(new AppError("type is required", StatusCodes.BAD_REQUEST));
    ...
    if (!duration) {
      return next(
        new AppError("duration is required", StatusCodes.BAD_REQUEST)
      );
    }
    ...
    // Parse flashcards data if it's a string
    let flashcardsArray;
    try {
      flashcardsArray =
        typeof flashcards === "string" ? JSON.parse(flashcards) : flashcards;
    } catch (error) {
      return next(
        new AppError("Invalid flashcards data format", StatusCodes.BAD_REQUEST)
      );
    }
    ...
    try {
      assessmentArray =
        typeof assessment === "string" ? JSON.parse(assessment) : assessment;
    } catch (error) {
      return next(
        new AppError("Invalid assessment data format", StatusCodes.BAD_REQUEST)
      );
    }
    ...
    try {
      ...
      return next(
        new AppError("Invalid content data format", StatusCodes.BAD_REQUEST)
      );
    }
    // console.log("content-------->>>:", contentArray);

    // Modify your assessmentData mapping:
    const assessmentData = assessmentArray.map(
      (question: any, index: number) => {
        ...
        return {
          ...
          questionSubHeading:
            question.questionSubHeading || question.subHeading || "",
    ...
    next(
      new AppError("Error updating module", StatusCodes.INTERNAL_SERVER_ERROR)
    );
  }
};

export const getModuleFlashcards = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    const flashcards = await questionService.getAllQuestions(
      module.flashcards || []
    );
    ...
    console.error("Error in getModuleFlashcards:", error);
    next(
      new AppError(
        "Error fetching module flashcards",
        StatusCodes.INTERNAL_SERVER_ERROR
      )
    );
  }
...
export const getCompleteModuleById = async (
  ...
  try {
    ...
    next(
      new AppError(
        "An unexpected error occurred while fetching the complete module",
        StatusCodes.INTERNAL_SERVER_ERROR
      )
    );
  }
...
export const deleteModuleById = async (
  ...
  try {
    ...
    const result = await moduleService.deleteModuleById(
      new mongoose.Types.ObjectId(moduleId)
    );
    ...
    next(
      new AppError("Error deleting module", StatusCodes.INTERNAL_SERVER_ERROR)
    );
  }
};
