...
} from "../services/quickkAi.service";
...
const validateModuleData = (moduleData: ModuleCreationRequest): void => {
  ...
  if (title.length > 100) {
    throw new AppError(
      "Module title must be less than 100 characters",
  ...
  if (!assessmentCount || assessmentCount < 1 || assessmentCount > 20) {
    throw new AppError(
      "Assessment count must be between 1 and 20",
      StatusCodes.BAD_REQUEST
    );
  }
  ...
  if (type === ModuleType.QUESTION) {
    if (!questionsCount || questionsCount < 1 || questionsCount > 20) {
      throw new AppError(
        "Questions count must be between 1 and 20 for question type modules",
        StatusCodes.BAD_REQUEST
      );
    }
  }
  ...
  if (moduleData.duration) {
    ...
    if (!durationRegex.test(moduleData.duration.trim())) {
      throw new AppError(
        'Duration format is invalid. Use format like "10 min" or "1 hr"',
        StatusCodes.BAD_REQUEST
      );
    }
  }
...
export const createAIModule = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    if(!updatedModule){
        throw new AppError("Failed to update module with generated content", StatusCodes.INTERNAL_SERVER_ERROR);
    }
    ...
    res.status(StatusCodes.CREATED).json({
      ...
      data: {
        ...
        contentCount:
          moduleData.type === ModuleType.VIDEO ? 1 : contentQuestions.length,
...
const validateDailyPulseData = (pulseData: DailyPulseCreationRequest): void => {
  ...
  if (!pulseCount || pulseCount < 1 || pulseCount > 10) {
    throw new AppError(
      "Pulse count must be between 1 and 10",
      StatusCodes.BAD_REQUEST
    );
  }
  ...
  if (!starsAwarded || starsAwarded < 1 || starsAwarded > 100) {
    throw new AppError(
      "Stars awarded must be between 1 and 100",
      StatusCodes.BAD_REQUEST
    );
  }
};

export const createAIDailyPulse = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    // Generate AI content
    const aiResponse = await aiDailyPulseService.generateDailyPulseContent(
      pulseData
    );
    console.log(
      "AI response received with",
      aiResponse.infoCards.length,
      "info cards and",
    ...
    const updatedDailyPulse = await dailyPulseService.updateDailyPulse(
      dailyPulseId,
      updateData
    );
...
export const getModuleCreationStatus = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
...
export const updateChatOnCompletion = async (req: Request, res: Response) => {
  try {
    ...
    if (createdModuleId)
      updateData.createdModuleId = new mongoose.Types.ObjectId(createdModuleId);
    if (createdDailyPulseId)
      updateData.createdDailyPulseId = new mongoose.Types.ObjectId(
        createdDailyPulseId
      );
...
export const improveEventDescription = async (
  ...
  try {
    ...
    const {
      ...
      eventTitle,
      eventType,
    }: DescriptionImprovementRequest = req.body;
    ...
    const improvedDescription =
      await aiDescriptionService.improveEventDescription({
        originalDescription,
        eventTitle,
        eventType,
      });
