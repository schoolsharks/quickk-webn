...
import UserOtpEmail from '../../../services/emails/triggers/user/otp-trigger';
...
export const sendOtp = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const user = await User.findOne({ companyMail });
        if (!user) {
            return next(new AppError("User not found. Please signup first.", StatusCodes.NOT_FOUND));
        }
        ...
        // Send OTP email
        await UserOtpEmail(otp, companyMail);
...
export const verifyOtp = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        if (!otpRecord) {
            return next(new AppError("OTP expired or not found. Please request a new OTP", StatusCodes.BAD_REQUEST));
        }
...
export const signup = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        if (!companyMail || !name || !businessName || !contact || !businessCategory) {
        ...
        if (existingUser) {
            return next(new AppError("User already exists. Please login instead.", StatusCodes.CONFLICT));
        }

        // Get the default company (WEBN Community)
        const defaultCompanyId = new mongoose.Types.ObjectId("67f7b486db92440936790acb");
        ...
        res.status(StatusCodes.CREATED).json({
            success: true,
            message: "Account created successfully! OTP sent to your email for login.",
            user: {
                id: newUser._id,
                name: newUser.name,
